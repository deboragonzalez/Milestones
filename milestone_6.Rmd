...
This PDF document present the working replication exercise of the afore-mentioned academic paper through a high-level visual presentation, a textual overview of the authors' findings and methods, and a close-replication of the original analysis. It also contains an extension proposal for future development of the findings.
...
Milestone #6. Submit a PDF or html via Canvas. 

In addition to the the elements from Milestone #5, your paper should include:

An Appendix in which you replicate all results — or all the important results — from your paper. 
...
Construct a model that controls for changes in hispanic and changes unemployment to consider the economic influences on gop_share shifts
Examine the results by state -- construct same models with subset of data by state
Examine whether there has been shifting from majority rep/dem with changes in hispanic population
...
fig1a <- sample_1 %>% 
  ...
  stat_smooth(method = "loess", formula = y ~ x, size = 0.5, color = "black", se = FALSE) +
...
# able to get the x and y axis limits.
...
sample_2 <- sample_n(new_precinct, 2000) %>% 
  mutate(d1600_hispanic <= quantile(d1611_hispanic,.95,na.rm=T) & 
         d1600_hispanic >= quantile(d1611_hispanic,.05,na.rm=T))
